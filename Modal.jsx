...
export default function Modal({ open, onClose, item, copied, onCopy }) {
  ...
  // Lock body scroll when modal is open and track product view
  useEffect(() => {
    if (open && item) {
      document.body.style.overflow = 'hidden'
      // Track product view
      const firstCategory = Array.isArray(item.category) ? item.category[0] : (item.category || 'unknown')
      trackProductView(item.title, firstCategory, item.brand)
    } else {
              ...
              <div className="modal-code">
                ...
                <div className="code-row">
                  ...
                  <button 
                    ...
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      trackDiscountCodeCopy(item.code, displayItem.title);
                      onCopy(item.code); 
                    }} 
              ...
              <div className="channel-links">
                {displayItem.links?.shopee && (
                  <a 
                    ...
                    onClick={() => {
                      ...
                      trackProductClick(displayItem.title, firstCategory, 'shopee', displayItem.links.shopee, displayItem.brand);
                      handleClose();
                    }}
                  >
                ...
                {displayItem.links?.tiktok && (
                  <a 
                    ...
                    onClick={() => {
                      const firstCategory = Array.isArray(displayItem.category) ? displayItem.category[0] : (displayItem.category || 'unknown');
                      trackProductClick(displayItem.title, firstCategory, 'tiktok', displayItem.links.tiktok, displayItem.brand);
                      handleClose();
                    }}
                  >
                ...
                {displayItem.links?.lazada && (
                  <a 
                    ...
                    onClick={() => {
                      const firstCategory = Array.isArray(displayItem.category) ? displayItem.category[0] : (displayItem.category || 'unknown');
                      trackProductClick(displayItem.title, firstCategory, 'lazada', displayItem.links.lazada, displayItem.brand);
                      handleClose();
                    }}
                  >
                ...
                {displayItem.links?.other && (
                  <a 
                    ...
                    onClick={() => {
                      const firstCategory = Array.isArray(displayItem.category) ? displayItem.category[0] : (displayItem.category || 'unknown');
                      trackProductClick(displayItem.title, firstCategory, 'other', displayItem.links.other, displayItem.brand);
                      handleClose();
                    }}
                  >
